...
## Off-site backups (belt and braces)

Wren makes automatic nightly database backups into `./backup`. Keep a copy
OFF the machine too — two easy options, use either or both:

### Option 1 — USB stick (no accounts, no internet)
```bash
# plug in a USB stick, then (adjust the mount path for your system):
cp backup/$(ls -t backup | head -1) /media/usb/wren-backup-$(date +%F).sql.gz
```
Do it weekly, keep the stick somewhere other than the nursery. Two sticks
rotated is even better.

### Option 2 — Google Drive (automatic, free 15GB)
One-time setup with [rclone](https://rclone.org):
```bash
curl https://rclone.org/install.sh | sudo bash
rclone config          # n) new remote → name: gdrive → type: drive → follow the browser login
```
Then add a nightly sync (runs at 02:30):
```bash
(crontab -l 2>/dev/null; echo "30 2 * * * rclone copy $(pwd)/backup gdrive:wren-backups --max-age 48h") | crontab -
```
Restore is the same in reverse: `rclone copy gdrive:wren-backups ./restore/`.

Test your restore once a term: `gunzip -c <backup file> | docker compose exec -T db psql -U wren -d wren` into a scratch database. A backup you've never restored is a hope, not a backup.
