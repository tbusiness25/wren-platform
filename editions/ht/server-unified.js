// Unified HT (head teacher school test) server
// Portal dispatch:
//   ht-roost.* → admin portal  (editions/admin/public)
//   ht-seed.*  → HR portal     (editions/hr/public)
//   ht-nest.*  → parents portal (editions/parents/public)
//   ht-egg.*   → learning      (editions/primary/public)
//   else       → learning
...
// The demo uses the nature-theme prefixes (ht-roost/seed/nest/egg). Real customer
// instances set INSTANCE_SLUG and use clear <slug>-admin/<slug>-hr/<slug>-parents
// prefixes (e.g. htps-admin.getwren.co.uk); their bare hostname is the learning
// portal. Both schemes are matched so one image serves every 4-portal instance.
const SLUG = (process.env.INSTANCE_SLUG || 'ht').toLowerCase();
app.use((req, res, next) => {
  const h = (req.hostname || '').toLowerCase();
  if      (h.startsWith('ht-roost.') || h.startsWith(SLUG + '-admin.'))   req._portal = 'admin';
  else if (h.startsWith('ht-seed.')  || h.startsWith(SLUG + '-hr.'))      req._portal = 'hr';
  else if (h.startsWith('ht-nest.')  || h.startsWith(SLUG + '-parents.')) req._portal = 'parents';
...
const PARENT_DENY = new Set([
  ...
  'risk-assessments','coshh','fire-safety','regulatory','inspection',
  'audit','admin','cockpit','finance','invoices','payments-admin','newsletter',
...
app.use((req, res, next) => {
  res.setHeader('X-Wren-Edition', 'ht-' + (req._portal || 'learning'));
  next();
});
...
const _staticHandlers = {
  ...
  learning: [
    express.static(path.join(__dirname, '../primary/public')),
...
app.use(require('helmet')({
  contentSecurityPolicy: {
    directives: {
      ...
      scriptSrc: ["'self'", "'unsafe-inline'"],
      styleSrc: ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com"],
      imgSrc: ["'self'", "data:", "blob:", "https://www.twinkl.co.uk", "https://*.twinkle.co.uk", "https://ik.imagekit.io"],
      connectSrc: ["'self'"],
      fontSrc: ["'self'", "data:", "https://fonts.gstatic.com"],
      objectSrc: ["'none'"],
      frameAncestors: ["'none'"],
...
try { app.use('/api/policies-ai',         require('../../src/routes/policies-ai')); } catch(e) { console.error('policies-ai:', e.message); }
try { app.use('/api/legislation',          require('../../src/routes/legislation')); } catch(e) { console.error('legislation:', e.message); }
...
// Import wizard (Tapestry / BrightHR / CSV / CTF) — lets a fresh instance load its own data
try { app.use('/api/import',             require('../../src/routes/import-wizard')); } catch(e) { console.error('mount import:', e.message); }
...
app.get('/api/edition', (req, res) => res.json({ edition: 'ht-unified', schema: SCHEMA }));

// ── Health ──
app.get('/health', (req, res) => res.json({ ok: true, edition: 'ht-unified', portal: req._portal }));

// ── Catch-all: serve login for non-API ──
app.use((req, res) => {
  if (req.path.startsWith('/api/')) return res.status(404).json({ error: 'Not found' });
  // Serve each portal's SPA entry (index.html) from its REAL edition public dir.
  // (The old map pointed at editions/ht/public — which is empty — and at filenames
  //  like admin.html/hr.html that don't exist, so every SPA route 404'd.)
  const dirMap = { admin: '../admin/public', hr: '../hr/public', parents: '../parents/public', learning: '../primary/public' };
  const base = dirMap[req._portal] || '../primary/public';
  res.sendFile(path.join(__dirname, base, 'index.html'));
});

const PORT = process.env.PORT || 3001;
app.listen(PORT, () => console.log(`Wren HT UNIFIED running on port ${PORT} (portals: admin/hr/parents/learning)`));
