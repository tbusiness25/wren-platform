;(function () {
  ...
  // Embedded iframes (Staff → Rota, Operations → Kitchen) must not spawn their own
  // chat FAB — the parent page already has one. Without this guard, pages that
  // hardcode a wren-chat.js <script> tag render two birds when iframed.
  ...
  const PERSONA_CONFIG = {
    ...
    apprentice: {
      name: 'Nestling',
      subtitle: 'Your early years learning buddy',
      greeting: "Hi, I'm Nestling 🐤 — your learning buddy for early years practice. Ask me anything about child development, attachment, the EYFS or how we care for children here. I only teach from our approved training and the EYFS framework, and I'll always show you my sources.",
      suggestions: [
        'Explain attachment theory simply',
        'What does co-regulation look like with a baby?',
        'How do I support a child who may be neurodivergent?',
        'Why does the key person approach matter?'
      ]
    }
  };
  ...
  let chatDragHandle = null;   // { hide, show } from makeDraggable — for Settings toggle
...
@media (max-width: 480px) {
  ...
  function buildWidget() {
    ...
    chatDragHandle = makeDraggable(btn, {
    ...
    // Greeting (persona-specific greeting wins over the generic default)
    addMessage('ai', p.greeting || cfg.greeting);
  }
  ...
  // Switch the live widget to a different persona (e.g. open Nestling from a page button).
  // Updates the header, suggestions and greeting, and resets the conversation context.
  function applyPersona(persona) {
    const p = PERSONA_CONFIG[persona] || PERSONA_CONFIG.eyfs;
    cfg.persona = persona;
    history = [];
    firstMessageSent = false;
    if (panel) {
      const nameEl = panel.querySelector('.wc-header-name');
      const subEl  = panel.querySelector('.wc-header-sub');
  ...
  function openPanel() {
    if (!panel) return;
    if (!isOpen) { isOpen = true; panel.classList.remove('hidden'); }
    // ensure the launcher isn't hidden behind the edge tab
    const btn = document.getElementById('wren-chat-btn');
    if (btn) btn.classList.remove('wc-removed');
  ...
  window.WrenChat = {
    ...
    // Switch persona on the already-loaded floating widget (builds it first if needed).
    setPersona(persona) {
      if (!document.getElementById('wren-chat-btn')) { cfg.persona = persona; buildWidget(); }
      else applyPersona(persona);
    },
    open() { openPanel(); },
    // Open the widget and send a prompt programmatically (used by page suggestion chips).
    ask(text) {
      const go = () => { openPanel(); if (input) { input.value = text; handleSend(); } };
      if (!document.getElementById('wren-chat-btn')) { buildWidget(); setTimeout(go, 60); }
      else go();
    },
    isReady() { return !!panel; },
    // ── Launcher visibility (used by Settings toggles) ──────────────────────────
    // Source of truth is the localStorage flag read on load; these also flip the
    // live launcher immediately when it's already on the page.
    setLauncherVisible(v) {
      if (v) { try { localStorage.removeItem('wren.chatBtnHidden'); } catch (e) {} }
      else   { try { localStorage.setItem('wren.chatBtnHidden', '1'); } catch (e) {} }
      if (chatDragHandle) { v ? chatDragHandle.show() : chatDragHandle.hide(); }
    },
    showLauncher() { this.setLauncherVisible(true); },
    hideLauncher() { this.setLauncherVisible(false); },
    isLauncherHidden() { try { return localStorage.getItem('wren.chatBtnHidden') === '1'; } catch (e) { return false; } }
  };

  document.addEventListener('wren:set-widget-visibility', function (e) {
    if (e && e.detail && e.detail.widget === 'chat') window.WrenChat.setLauncherVisible(!!e.detail.visible);
  });
