...
@media (max-width: 480px) {
  ...
  async function startRecording() {
    ...
    if (!window.isSecureContext || !navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      setStatus(!window.isSecureContext
        ? '⚠ Microphone needs a secure (https://) connection — open the portal via its https web address, not an IP.'
    ...
    try {
      ...
      if (err.name === 'NotAllowedError') {
        setStatus('⚠ Microphone permission needed — check browser settings');
        panel.querySelector('#vc-hint').textContent = '';
      } else {
  ...
  async function upload(blob, mime, context) {
    try {
      ...
      setStatus('Sent for processing ✓');
      toast('Voice note sent for processing', 'success');
      setTimeout(() => { if (isOpen) closePanel(); }, 1500);
    } catch {
