...
// Nursery identity for PDF header/footer. Demo editions (DEMO_MODE/AI_DEMO_MODE)
// show neutral placeholders so prospect-facing demos never expose a real setting's
// name, address or phone. Production (no DEMO flag) keeps the configured identity.
const IS_DEMO = process.env.DEMO_MODE === 'true' || process.env.AI_DEMO_MODE === 'true';
const NURSERY = IS_DEMO
...
function drawHeader(doc, subtitle) {
  ...
  doc.fillColor(C_BLUE).font('Helvetica-Bold').fontSize(16)
     .text(NURSERY.name1, textX, 20, { continued: true })
     .fillColor(C_ORANGE)
     .text(NURSERY.name2, { lineBreak: false });

  // Address sub-line — position at y=44 inside header
  doc.fillColor('#94a3b8').font('Helvetica').fontSize(8)
     .text(NURSERY.headerLine,
