...
// cockpit kanban (ladn.cockpit_cards) when a staff-HR signal crosses a threshold. The
// manager then actions the card from Hawk. This is the same surface Hermes posts to, so the
// board is a single inbox for both human-tap and agent/automation findings.
//
// MONITORS:
//   1. Absence (Bradford Factor) — rolling-12m Bradford per active staff from ladn.hr_absences,
...
async function runAbsenceMonitor(open) {
  ...
  const { rows } = await db.query(`
    WITH sick AS (
      ...
      JOIN ladn.staff s ON s.id = a.staff_id AND s.is_active AND NOT COALESCE(s.terminated, false)
    ...
    FROM agg a JOIN ladn.staff s ON s.id = a.staff_id
...
async function runSupervisionMonitor(open) {
  ...
  const { rows } = await db.query(`
    WITH latest AS (
      ...
      JOIN ladn.staff s ON s.id = sup.staff_id AND s.is_active AND NOT COALESCE(s.terminated, false)
    ...
    SELECT o.staff_id, o.overdue_since,
           ...
           s.first_name, s.last_name
    FROM overdue o JOIN ladn.staff s ON s.id = o.staff_id
