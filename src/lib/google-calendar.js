...
// Get or create the dedicated Wren Nursery calendar, caching ID in settings
async function getCalendarId(db) {
  // Check settings
  const { rows } = await db.query(
    `SELECT value FROM settings WHERE key = 'gcal_calendar_id'`
  );
...
async function createEvent(db, { summary, description, start, end, location, wrenRef, wrenType, colorId }) {
  ...
  if (wrenRef && db) {
    await db.query(
      `INSERT INTO gcal_events (wren_ref, gcal_event_id, calendar_id, wren_type, synced_at, updated_at)
...
async function updateEvent(db, wrenRef, fields) {
  const { rows } = await db.query(
    `SELECT gcal_event_id, calendar_id FROM gcal_events WHERE wren_ref = $1`, [wrenRef]
  );
  ...
  await db.query(`UPDATE gcal_events SET updated_at = now() WHERE wren_ref = $1`, [wrenRef]);
  return res.data;
}
...
async function deleteEvent(db, wrenRefOrEventId) {
  ...
  const { rows } = await db.query(
    `SELECT gcal_event_id, calendar_id FROM gcal_events WHERE wren_ref = $1`, [wrenRefOrEventId]
  );
  if (rows.length) {
  ...
  await db.query(`DELETE FROM gcal_events WHERE wren_ref = $1 OR gcal_event_id = $2`, [wrenRefOrEventId, eventId]);
  return { ok: true };
}
