 ...
 */
// Log every outbound email Wren sends into ladn.email_audit (previously logged nowhere).
async function _auditEmail(to, subject, eventType, source, ok, error) {
  try {
    const { getPool } = require('../db/pool');
    await getPool().query(
      `INSERT INTO ladn.email_audit (direction, to_emails, subject, event_type, source, sent_ok, error)
