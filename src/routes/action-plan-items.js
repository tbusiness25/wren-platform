...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(`
      ...
      LEFT JOIN ladn.staff cs ON cs.id = i.completed_by_staff_id
      WHERE i.plan_id = $1
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const { rows: plan } = await db.query(
      'SELECT id, title, scope FROM ladn.action_plans WHERE id=$1 AND archived_at IS NULL', [plan_id]
    );
    if (!plan.length) return res.status(404).json({ error: 'Plan not found' });
    const { rows } = await db.query(`
      INSERT INTO ladn.action_plan_items
    ...
    if (assigned_staff_id) {
      const { rows: st } = await db.query(
        'SELECT telegram_chat_id, first_name FROM ladn.staff WHERE id=$1', [assigned_staff_id]
      );
    ...
    // Audit
    await db.query(`INSERT INTO ladn.action_plan_audit (plan_id,item_id,actor_id,actor_type,action,after_value)
...
router.patch('/:id', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(
      `UPDATE ladn.action_plan_items SET ${updates.join(',')} WHERE id=$${vals.length} RETURNING *`, vals
    );
    ...
    if (req.body.assigned_staff_id && req.body.assigned_staff_id !== item.assigned_staff_id) {
      const { rows: st } = await db.query(
        'SELECT telegram_chat_id, first_name FROM ladn.staff WHERE id=$1', [req.body.assigned_staff_id]
      );
      if (st[0]?.telegram_chat_id) {
        await sendTelegramTo(st[0].telegram_chat_id,
    ...
    await db.query(`INSERT INTO ladn.action_plan_audit (plan_id,item_id,actor_id,actor_type,action,before_value,after_value)
