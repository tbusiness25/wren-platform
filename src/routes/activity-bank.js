...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const { rows: countRows } = await db.query(
      `SELECT COUNT(*) FROM ladn.planning_activities ${where}`,
      params.slice(0, -2)
    );
  ...
  try {
    ...
    const { rows } = await db.query(`
      SELECT
        ...
        (SELECT ARRAY_AGG(DISTINCT e ORDER BY e) FROM ladn.planning_activities, UNNEST(eyfs_areas) e) as eyfs_areas,
        (SELECT ARRAY_AGG(DISTINCT t ORDER BY t) FROM ladn.planning_activities, UNNEST(tags) t) as all_tags,
        (SELECT ARRAY_AGG(DISTINCT ed ORDER BY ed) FROM ladn.planning_activities, UNNEST(edition) ed) as editions,
        (SELECT ARRAY_AGG(DISTINCT ab ORDER BY ab) FROM ladn.planning_activities, UNNEST(age_band) ab) as age_bands
      FROM ladn.planning_activities
    `);
    res.json(rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
...
router.get('/:id', async (req, res) => {
  try {
    const db = getPool();
    const { rows } = await db.query('SELECT * FROM ladn.planning_activities WHERE id=$1', [req.params.id]);
...
router.post('/', async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows } = await db.query(`
      INSERT INTO ladn.planning_activities (
        title, description, age_group, duration_minutes, eyfs_areas, learning_objectives,
        materials_needed, setup_instructions, step_by_step, extension_ideas,
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows } = await db.query(
      `UPDATE ladn.planning_activities SET ${updates.join(',')} WHERE id=$${vals.length} RETURNING *`, vals
    );
    res.json(rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    await getPool().query('DELETE FROM ladn.planning_activities WHERE id=$1', [req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/:id/use', async (req, res) => {
  try {
    const db = getPool();
    const { rows } = await db.query(`
      UPDATE ladn.planning_activities
...
router.post('/:id/favourite', async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows } = await db.query('SELECT favourited_by FROM ladn.planning_activities WHERE id=$1', [req.params.id]);
    if (!rows.length) return res.status(404).json({ error: 'Not found' });
    ...
    await db.query('UPDATE ladn.planning_activities SET favourited_by=$1 WHERE id=$2', [newFavs, req.params.id]);
...
router.post('/:id/share', async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows } = await db.query(
      'UPDATE ladn.planning_activities SET share_to_community=NOT COALESCE(share_to_community,false), updated_at=NOW() WHERE id=$1 RETURNING share_to_community',
      [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Not found' });
