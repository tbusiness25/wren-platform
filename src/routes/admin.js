...
router.post('/sync-cf', managerOnly, async (req, res) => {
  try {
    return res.status(503).json({ error: 'CF allowlist sync disabled 2026-04-20 — manual list is authoritative. Re-enable in admin.js + children.js when child data is live.' });
...
router.get('/waiting-list-count', async (req, res) => {
  try {
    const db = getPool();
    const { rows } = await db.query("SELECT COUNT(*) as count FROM waiting_list WHERE status='active'");
