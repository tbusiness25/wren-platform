...
async function upsertContact(db, { email, phone, name, status, enquiryId }) {
  ...
  if (emailLower) {
    const { rows } = await db.query(
      `SELECT id FROM ladn.contacts WHERE lower(primary_email)=$1`, [emailLower]
    );
    if (rows.length) {
      await db.query(
        `UPDATE ladn.contacts SET
    ...
    const { rows } = await db.query(
      `SELECT id FROM ladn.contacts WHERE primary_email IS NULL AND primary_phone=$1`, [phone]
    );
    if (rows.length) return rows[0].id;
  }
  const { rows } = await db.query(
    `INSERT INTO ladn.contacts
  ...
  await db.query(
    `INSERT INTO ladn.contact_status_history (contact_id,to_status,changed_at,notes)
...
async function upsertThread(db, contactId, subject) {
  const { rows } = await db.query(
    `SELECT id FROM ladn.threads WHERE contact_id=$1 ORDER BY created_at LIMIT 1`, [contactId]
  );
  if (rows.length) return rows[0].id;
  const { rows: ins } = await db.query(
    `INSERT INTO ladn.threads (contact_id,subject,created_at) VALUES ($1,$2,now()) RETURNING id`,
...
async function insertThreadMessage(db, { threadId, direction, source, bodyText,
  ...
  await db.query(
    `INSERT INTO ladn.thread_messages
...
router.get('/', async (req, res) => {
  try {
    ...
    const { rows } = await db.query(`
      ...
      FROM ladn.contacts c
      LEFT JOIN ladn.threads t ON t.contact_id=c.id
      ${where}
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const { rows: [contact] } = await db.query(
      `SELECT *, COALESCE(full_name,primary_email,primary_phone) AS display_name FROM ladn.contacts WHERE id=$1`, [id]
    );
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [{ rows: [contact] }, { rows: threads }] = await Promise.all([
      db.query(`SELECT * FROM ladn.contacts WHERE id=$1`, [req.params.id]),
      db.query(`SELECT * FROM ladn.threads WHERE contact_id=$1 ORDER BY created_at`, [req.params.id]),
    ]);
    ...
    if (primaryThread) {
      const { rows } = await db.query(`
        ...
        FROM ladn.thread_messages tm
        LEFT JOIN ladn.staff s ON s.id=tm.sent_by_staff_id
    ...
    const { rows: history } = await db.query(`
      ...
      FROM ladn.contact_status_history csh
      LEFT JOIN ladn.staff s ON s.id=csh.changed_by
...
router.patch('/:id/status', async (req, res) => {
  ...
  try {
    ...
    const { rows: [cur] } = await db.query(
      `SELECT status FROM ladn.contacts WHERE id=$1`, [req.params.id]
    );
    ...
    const { rows: threads } = await db.query(
      `SELECT id FROM ladn.threads WHERE contact_id=$1 LIMIT 1`, [req.params.id]
    );
    if (threads.length) {
...
router.post('/:id/spam', async (req, res) => {
  try {
    const db = getPool();
    const { rows: [contact] } = await db.query(
      `SELECT * FROM ladn.contacts WHERE id=$1`, [req.params.id]
    );
    if (!contact) return res.status(404).json({ error: 'Not found' });
    ...
    if (domain) {
      await db.query(
        `INSERT INTO ladn.email_sender_rules (pattern, rule, reason, created_at)
         VALUES ($1,'never-alert',$2,now()) ON CONFLICT DO NOTHING`,
        [domain, `Marked as spam by staff (contact ID ${contact.id})`]
      );
    }
    await db.query(
      `UPDATE ladn.contacts SET archived_at=now() WHERE id=$1`, [req.params.id]
    );
...
router.post('/:id/archive', async (req, res) => {
  try {
    const db = getPool();
    await db.query(`UPDATE ladn.contacts SET archived_at=now() WHERE id=$1`, [req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/thread/:threadId/read', async (req, res) => {
  try {
    const db = getPool();
    await db.query(
      `UPDATE ladn.thread_messages SET is_read=true, read_at=now()
       WHERE thread_id=$1 AND is_read=false`, [req.params.threadId]
    );
    await db.query(
      `UPDATE ladn.threads SET unread_count=0 WHERE id=$1`, [req.params.threadId]
    );
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
