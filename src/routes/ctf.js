...
async function initSchema() {
  ...
  const childCols = [
    ...
    "ALTER TABLE children ADD COLUMN IF NOT EXISTS service_child   BOOLEAN DEFAULT FALSE",
    "ALTER TABLE children ADD COLUMN IF NOT EXISTS nationality     TEXT",
    "ALTER TABLE children ADD COLUMN IF NOT EXISTS country_of_birth TEXT",
    ...
    "ALTER TABLE children ADD COLUMN IF NOT EXISTS preferred_surname   TEXT",
    "ALTER TABLE children ADD COLUMN IF NOT EXISTS preferred_forename  TEXT",
    "ALTER TABLE children ADD COLUMN IF NOT EXISTS nc_year         TEXT",
    "ALTER TABLE children ADD COLUMN IF NOT EXISTS sen_status      VARCHAR(1)",
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS ctf_sen_history (
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS ctf_fsm_history (
      id              SERIAL PRIMARY KEY,
      child_id        INTEGER NOT NULL,
      fsm_start_date  DATE,
      fsm_end_date    DATE,
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS ctf_school_history (
      id              SERIAL PRIMARY KEY,
      child_id        INTEGER NOT NULL,
      lea             TEXT,
      estab           TEXT,
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS ctf_assessment_results (
      id               SERIAL PRIMARY KEY,
      child_id         INTEGER NOT NULL,
      stage            TEXT,
  ...
  await db.query(`CREATE INDEX IF NOT EXISTS children_upn_idx ON children(upn)
...
router.get('/import/jobs/:id', async (req, res) => {
  try {
    const { rows } = await getPool().query(
      'SELECT id, source_kind, status, row_count_total, row_count_imported, uploaded_at, updated_at, error_log_json FROM import_jobs WHERE id=$1',
      [req.params.id]);
...
router.post('/export/generate', async (req, res) => {
  ...
  try {
    ...
    const { rows: settings } = await db.query(
      `SELECT value FROM settings WHERE key IN ('school_lea','school_estab','school_name') LIMIT 10`
    ).catch(() => ({ rows: [] }));
    ...
    const { rows: children } = await db.query(
      `SELECT * FROM children WHERE id IN (${placeholders})`, ids);
    ...
    for (const child of children) {
      const [senH, senP, fsmH, assR, schH] = await Promise.all([
        db.query('SELECT * FROM ctf_sen_history WHERE child_id=$1 ORDER BY stage_start_date', [child.id]),
        db.query('SELECT * FROM sen_register WHERE child_id=$1 AND is_active=true LIMIT 10', [child.id]),
        db.query('SELECT * FROM ctf_fsm_history WHERE child_id=$1 ORDER BY fsm_start_date', [child.id]),
        db.query('SELECT * FROM ctf_assessment_results WHERE child_id=$1 ORDER BY year, subject_code', [child.id]),
        db.query('SELECT * FROM ctf_school_history WHERE child_id=$1 ORDER BY entry_date', [child.id]),
      ]);
...
router.get('/export/:id/download', async (req, res) => {
  try {
    const db = getPool();
    const { rows } = await db.query('SELECT * FROM ctf_exports WHERE id=$1', [req.params.id]);
  ...
  try {
    const { rows } = await getPool().query(
      `SELECT id, exported_at, child_count, qualifier, dest_school_name, filename
       FROM ctf_exports ORDER BY exported_at DESC LIMIT 50`);
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
async function findExistingChild(db, p) {
  ...
  if (p.upn && !p.upn_is_temp) {
    const { rows } = await db.query(
      'SELECT id FROM children WHERE upn=$1', [p.upn]);
  ...
  for (const p of pupils) {
    try {
      ...
      for (const s of p.sen_history) {
        ...
        await client.query(
          `INSERT INTO ctf_sen_history (child_id, stage_type, stage_start_date)
           SELECT $1,$2,$3
           WHERE NOT EXISTS (
             SELECT 1 FROM ctf_sen_history
      ...
      for (const f of p.fsm_history) {
        ...
        await client.query(
          `INSERT INTO ctf_fsm_history (child_id, fsm_start_date, fsm_end_date, fsm_eligible, fsm_uk_born)
           SELECT $1,$2,$3,$4,$5
           WHERE NOT EXISTS (
             SELECT 1 FROM ctf_fsm_history
      ...
      for (const a of p.assessment_results) {
        ...
        await client.query(
          `INSERT INTO ctf_assessment_results
  ...
  try {
    const { rows } = await getPool().query(
      `SELECT id, first_name, last_name, date_of_birth, upn, year_group, nc_year,
       ...
       FROM children WHERE is_active=true ORDER BY last_name, first_name`);
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
