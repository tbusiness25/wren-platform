  ...
  try {
    ...
    const { rows } = await pool().query(`
      SELECT t.id, t.message_id, t.thread_id, t.received_at, t.from_email, t.from_name,
             ...
             EXISTS(SELECT 1 FROM ladn.email_triage_feedback f WHERE f.triage_id = t.id) AS has_feedback
      FROM ladn.email_triage t
      ORDER BY t.received_at DESC
      LIMIT $1 OFFSET $2
    `, [limit, offset]);
    const { rows: [{ total }] } = await pool().query('SELECT COUNT(*) as total FROM ladn.email_triage');
  ...
  try {
    const { rows: [s] } = await pool().query(`
      SELECT
        ...
        (SELECT COUNT(*)::int FROM ladn.email_triage_feedback f
           JOIN ladn.email_triage t ON f.triage_id = t.id
...
router.get('/rules/list', authenticate, async (req, res) => {
  try {
    const { rows } = await pool().query(
      'SELECT * FROM ladn.email_sender_rules ORDER BY id'
    );
    res.json(rows);
  } catch (e) {
    res.status(500).json({ error: e.message });
...
router.post('/rules/list', authenticate, async (req, res) => {
  ...
  try {
    const { rows } = await pool().query(
      'INSERT INTO ladn.email_sender_rules (pattern, rule, reason) VALUES ($1, $2, $3) RETURNING *',
...
router.delete('/rules/:id', authenticate, async (req, res) => {
  try {
    await pool().query('DELETE FROM ladn.email_sender_rules WHERE id=$1', [req.params.id]);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    const { rows } = await pool().query(
      'SELECT * FROM ladn.email_triage WHERE id = $1', [req.params.id]
    );
...
router.post('/:id/feedback', authenticate, async (req, res) => {
  ...
  try {
    await pool().query(
      'INSERT INTO ladn.email_triage_feedback (triage_id, correction, detail) VALUES ($1, $2, $3)',
...
router.patch('/:id/user-action', authenticate, async (req, res) => {
  ...
  try {
    await pool().query(
      'UPDATE ladn.email_triage SET user_action=$1, user_action_at=NOW() WHERE id=$2',
...
router.post('/:id/reclassify', authenticate, async (req, res) => {
  try {
    const { rows } = await pool().query(
      'SELECT id, from_email, from_name, subject, body_preview FROM ladn.email_triage WHERE id=$1',
      [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Not found' });
    const em = rows[0];
...
Body: ${(em.body_preview || '').substring(0, 800)}`;
    ...
    await pool().query(
      `UPDATE ladn.email_triage SET category=$1, importance=$2, sender_type=$3, summary=$4,
