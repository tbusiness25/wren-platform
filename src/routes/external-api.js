...
async function extAuth(req, res, next) {
  ...
  try {
    ...
    const { rows: [tok] } = await db.query(`
      ...
      JOIN parent_portal_access pa ON lower(pa.email) = lower(t.parent_email) AND pa.is_active = true
    ...
    await db.query('UPDATE external_api_tokens SET last_used_at=NOW() WHERE token_hash=$1', [hash]);
    req.extToken = tok;
    next();
  } catch (e) {
...
async function getEffectivePerms(db, childId) {
  const { rows } = await db.query(`
    SELECT m.attribute_key,
           COALESCE(
             (SELECT co.api FROM parent_permissions_child_overrides co
  ...
  try {
    ...
    if (perms.daily_diary) {
      const { rows: [diary] } = await db.query(`
        SELECT mood, food_ate, food_rating, nappy_count FROM daily_diary
    ...
    if (perms.nap) {
      const { rows: naps } = await db.query(`
        SELECT start_time, end_time, duration_minutes FROM sleep_checks
    ...
    if (perms.attendance) {
      const { rows: [att] } = await db.query(`
        SELECT status, arrived_at FROM attendance
    ...
    if (perms.at_setting) {
      const { rows: [clk] } = await db.query(`
        SELECT clock_in_time, clock_out_time FROM staff_clock_events
    ...
    if (perms.fees) {
      const { rows: [fee] } = await db.query(`
        ...
        FROM invoices WHERE child_id=$1 AND status NOT IN ('paid','cancelled')
      `, [childId]);
    ...
    if (perms.next_event) {
      const { rows: [ev] } = await db.query(`
        SELECT title, event_date, event_type FROM curriculum_activities
    ...
    if (perms.homework_due) {
      const { rows: hw } = await db.query(`
        SELECT title, due_date, subject FROM homework
    ...
    if (perms.behaviour_points) {
      const { rows: [bp] } = await db.query(`
        SELECT COALESCE(SUM(points),0)::int AS total FROM behaviour_points
...
router.post('/v1/parent/tokens', async (req, res) => {
  ...
  try {
    ...
    const { rows: [access] } = await db.query(
      'SELECT child_id FROM parent_portal_access WHERE lower(email)=$1 AND child_id=$2 AND is_active=true',
      [email, child_id]
    );
    ...
    const { rows: existing } = await db.query(
      'SELECT id FROM external_api_tokens WHERE lower(parent_email)=$1 AND child_id=$2 AND revoked_at IS NULL',
      [email, child_id]
    );
    ...
    await db.query(`
      INSERT INTO external_api_tokens (token_hash, parent_email, child_id, label)
      VALUES ($1, $2, $3, $4)
...
router.delete('/v1/parent/tokens/:id', async (req, res) => {
  ...
  try {
    const { rowCount } = await db.query(
      'UPDATE external_api_tokens SET revoked_at=NOW() WHERE id=$1 AND lower(parent_email)=$2 AND revoked_at IS NULL',
      [req.params.id, email]
    );
...
staffRouter.delete('/admin/tokens/:id', async (req, res) => {
  ...
  const db = getPool();
  try {
    await db.query('UPDATE external_api_tokens SET revoked_at=NOW() WHERE id=$1', [req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
