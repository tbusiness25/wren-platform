...
router.post('/push/outing/:id', async (req, res) => {
  try {
    ...
    const { rows: [outing] } = await db.query(
      `SELECT * FROM ladn.outings WHERE id = $1`, [req.params.id]
    );
...
router.post('/push/enquiry-tour/:id', async (req, res) => {
  try {
    ...
    const { rows: [enq] } = await db.query(
      `SELECT * FROM ladn.enquiries WHERE id = $1`, [req.params.id]
    );
...
router.get('/synced', async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      `SELECT * FROM ladn.gcal_events ORDER BY synced_at DESC LIMIT 200`
    );
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
