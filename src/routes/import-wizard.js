...
async function initSchema() {
  const db = getPool();
  await db.query(`
    CREATE TABLE IF NOT EXISTS ladn.import_jobs (
  ...
  `);
  await db.query(`
    CREATE TABLE IF NOT EXISTS ladn.import_templates (
...
router.post('/jobs/:id/mapping', async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows } = await db.query('SELECT id FROM ladn.import_jobs WHERE id=$1', [req.params.id]);
    if (!rows.length) return res.status(404).json({ error: 'Job not found' });
    ...
    await db.query(
      `UPDATE ladn.import_jobs SET mapping_json=$1, status='mapped', updated_at=NOW()${setEntity} WHERE id=$2`,
      params);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/jobs/:id/validate', async (req, res) => {
  ...
  try {
    ...
    await db.query(
      `UPDATE ladn.import_jobs SET status=$1, row_count_total=$2, error_log_json=$3, updated_at=NOW() WHERE id=$4`,
...
router.post('/jobs/:id/dry-run', async (req, res) => {
  ...
  try {
    ...
    await db.query(
      `UPDATE ladn.import_jobs SET row_count_total=$1, updated_at=NOW() WHERE id=$2`,
...
router.get('/jobs/:id/errors.csv', async (req, res) => {
  try {
    const { rows } = await getPool().query(
      'SELECT error_log_json FROM ladn.import_jobs WHERE id=$1', [req.params.id]);
...
router.get('/templates', async (req, res) => {
  try {
    const { rows } = await getPool().query(
      'SELECT * FROM ladn.import_templates ORDER BY is_builtin DESC, created_at DESC');
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/templates', async (req, res) => {
  ...
  try {
    const { rows } = await getPool().query(
      `INSERT INTO ladn.import_templates (name, source_kind, target_entity, mapping_json, created_by)
...
async function seedBuiltinTemplates() {
  ...
  for (const tpl of templates) {
    try {
      await db.query(
        `INSERT INTO ladn.import_templates (name, source_kind, target_entity, mapping_json, version, is_builtin)
...
router.post('/tapestry/:id/upload', (req, res) => {
  uploadZip.single('file')(req, res, async (err) => {
    ...
    try {
      await getPool().query(
        `UPDATE ladn.import_jobs SET file_path=$1, status='uploaded', uploaded_at=NOW(), updated_at=NOW() WHERE id=$2`,
        [req.file.path, req.params.id]);
...
router.post('/tapestry/:id/commit', async (req, res) => {
  try {
    ...
    try {
      ...
      try { await client.query('ROLLBACK'); } catch (_) {}
      await db.query(`UPDATE ladn.import_jobs SET status='failed', updated_at=NOW() WHERE id=$1`, [job.id]);
      throw err;
    } finally { client.release(); }

    const summary = tapSummarise(result);
    await db.query(
      `UPDATE ladn.import_jobs SET status='committed', row_count_imported=$1, error_log_json=$2, updated_at=NOW() WHERE id=$3`,
...
router.post('/brighthr/:id/upload/absences', (req, res) => {
  makeBrightHRUpload('absences.csv').single('file')(req, res, async (err) => {
    ...
    try {
      ...
      await getPool().query(
        `UPDATE ladn.import_jobs SET mapping_json=jsonb_set(COALESCE(mapping_json,'{}'),'{"absences_uploaded"}','"true"'), updated_at=NOW() WHERE id=$1`,
        [req.params.id]);
...
router.get('/brighthr/:id/preview', async (req, res) => {
  try {
    const { rows } = await getPool().query('SELECT * FROM ladn.import_jobs WHERE id=$1', [req.params.id]);
    if (!rows.length) return res.status(404).json({ error: 'Job not found' });
    const job = rows[0];
...
router.post('/brighthr/:id/commit', async (req, res) => {
  try {
    ...
    await db.query(
      `UPDATE ladn.import_jobs SET status='committed', row_count_imported=$1, row_count_total=$2, error_log_json=$3, updated_at=NOW() WHERE id=$4`,
