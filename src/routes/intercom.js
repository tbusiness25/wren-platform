...
router.get('/events/active', authenticate, _requireStaff, async (req, res) => {
  ...
  try {
    const { rows } = await db.query(`
      ...
      FROM ladn.doorbell_events de
      LEFT JOIN ladn.staff s ON s.id = de.answered_by_staff_id
...
router.get('/events/:id', authenticate, _requireStaff, async (req, res) => {
  ...
  try {
    const { rows } = await db.query(`
      ...
      FROM ladn.doorbell_events de
      LEFT JOIN ladn.staff s1 ON s1.id = de.answered_by_staff_id
      LEFT JOIN ladn.staff s2 ON s2.id = de.door_released_by_staff_id
      WHERE de.id = $1
    `, [req.params.id]);
...
router.post('/events/:id/release-door', authenticate, _requireStaff, async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(`
      SELECT * FROM ladn.doorbell_events
      WHERE id=$1
...
router.get('/ha-config', authenticate, async (req, res) => {
  ...
  const db = getPool();
  try {
    const { rows } = await db.query('SELECT key, entity_id, notes, updated_at FROM ladn.ha_config ORDER BY key');
    res.json(rows);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
router.put('/ha-config/:key', authenticate, async (req, res) => {
  ...
  try {
    await db.query(
      `INSERT INTO ladn.ha_config (key, entity_id, updated_at) VALUES ($1,$2,NOW())
