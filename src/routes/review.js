'use strict';
// review.js — the Review / autonomy queue API (Roost admin portal).
//
...
// The manager Approves / Edits / Rejects. EVERY verdict is logged to decision_log via
// src/lib/decision-log.js with a scenario fingerprint, which feeds scripts/confidence-update.js ->
// decision_confidence. That is the "it remembers" learning hook — repeated consistent verdicts
...
router.get('/decisions', requireManager, async (req, res) => {
  try {
    ...
    const { rows: decisions } = await db.query(`
      ...
      LEFT JOIN staff s ON s.id = dl.decided_by_staff_id
      LEFT JOIN decision_confidence dc
