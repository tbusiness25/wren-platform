...
router.get('/work-patterns', requireRole('manager', 'deputy_manager'), async (req, res) => {
  try {
    const { rows } = await getPool().query(`
      ...
      FROM ladn.staff_work_patterns wp
      JOIN ladn.staff s ON s.id = wp.staff_id
...
//
// 🔒 CRITICAL: for id=1 (Toby) the PIN must be written to BOTH ladn.staff
// AND ladn.protected_staff_pins in the SAME transaction so the backup never
...
router.post('/:id/set-pin', async (req, res) => {
  ...
  try {
    ...
    const { rows: upd } = await client.query(
      'UPDATE ladn.staff SET pin_hash=$1, pin_length=$2, updated_at=NOW() WHERE id=$3 RETURNING id, first_name, last_name',
    ...
    // id=1 PIN-sync rule — keep the protected backup in lockstep
    if (targetId === 1) {
...
router.patch('/:id/deactivate', requireRole('manager', 'deputy_manager'), async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows } = await db.query(
      'UPDATE ladn.staff SET is_active=false, updated_at=NOW() WHERE id=$1 RETURNING id, first_name, last_name, is_active',
      [targetId]
    );
...
router.patch('/:id/reactivate', requireRole('manager', 'deputy_manager'), async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows } = await db.query(
      'UPDATE ladn.staff SET is_active=true, updated_at=NOW() WHERE id=$1 RETURNING id, first_name, last_name, is_active',
      [targetId]
    );
    if (!rows.length) return res.status(404).json({ error: 'Staff not found' });
...
router.post('/import-workpattern', requireRole('manager', 'deputy_manager'), (req, res, next) => {
  ...
  for (const row of dataRows) {
    ...
    const { rows: staffRows } = await db.query(
      'SELECT id FROM ladn.staff WHERE lower(first_name)=$1 AND lower(last_name)=$2 AND is_active=true',
    ...
    if (staffUpdates.length) {
      ...
      await db.query(`UPDATE ladn.staff SET ${staffUpdates.join(',')}, updated_at=NOW() WHERE id=$${staffVals.length}`, staffVals);
    }
