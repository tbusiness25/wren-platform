...
async function getTermDates(db) {
  try {
    const { rows } = await db.query(
      "SELECT value FROM ladn.wren_settings WHERE key='term_dates_2025_2026'"
    );
...
async function computeForecast(db, dateStr) {
  ...
  const { rows: rooms } = await db.query(
    'SELECT id, name, display_name, capacity, min_age_months, max_age_months FROM ladn.rooms ORDER BY id'
  );
  ...
  const { rows: rotaStaff } = await db.query(`
    ...
    FROM ladn.rota_shifts rs
    JOIN ladn.staff s ON s.id = rs.staff_id
  ...
  const { rows: absences } = await db.query(`
    ...
    FROM ladn.absence_requests ar
    JOIN ladn.staff s ON s.id = ar.staff_id
  ...
  if (isPast) {
    const { rows: ac } = await db.query(
      'SELECT COUNT(*) AS cnt FROM ladn.attendance WHERE date = $1::date', [dateStr]
    );
...
router.get('/forecast', managerOnly, async (req, res) => {
  ...
  try {
    ...
    if (isPast) {
      const { rows } = await db.query(
        'SELECT payload FROM ladn.state_forecast_cache WHERE forecast_date = $1', [dateStr]
      );
    ...
    if (!payload) {
      ...
      if (isPast) {
        await db.query(
          `INSERT INTO ladn.state_forecast_cache (forecast_date, computed_at, payload)
           VALUES ($1, NOW(), $2)
