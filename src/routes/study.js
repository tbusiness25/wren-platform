...
router.post('/attempt/start', async (req, res) => {
  ...
  try {
    const { rows } = await db.query(`
      INSERT INTO parent_module_attempts (parent_email, module_id, child_name, started_at)
      VALUES ($1,$2,$3,NOW())
      RETURNING id
...
router.post('/attempt/complete', async (req, res) => {
  ...
  try {
    ...
    try {
      const childRow = await db.query('SELECT child_name FROM parent_module_attempts WHERE id=$1', [attempt_id]);
...
async function computeRewardProgress(db, email) {
  const setRes = await db.query(
    `SELECT rs.module_id, m.slug, m.title, rs.position
     FROM parent_guide_reward_set rs
     JOIN parent_study_modules m ON m.id = rs.module_id
  ...
  if (email) {
    const compRes = await db.query(
      `SELECT DISTINCT module_id FROM parent_module_attempts
...
async function maybeIssueCredit(db, email, childName) {
  ...
  try {
    const { rows } = await db.query(`
      INSERT INTO parent_account_credits
...
router.post('/reward/claim', async (req, res) => {
  ...
  try {
    ...
    const attempt = await db.query(`
      SELECT id FROM parent_module_attempts
    ...
    const mod = await db.query(
      'SELECT id, title, category, age_group FROM parent_study_modules WHERE id=$1',
      [module_id]
    );
    ...
    const { rows } = await db.query(`
      INSERT INTO parent_rewards (parent_email, child_name, module_id, reward_type, reward_data, earned_at)
...
router.get('/reward/mine', async (req, res) => {
  ...
  try {
    const { rows } = await db.query(`
      ...
      FROM parent_rewards r
      LEFT JOIN parent_study_modules m ON m.id = r.module_id
...
router.get('/reward/:id/pdf', async (req, res) => {
  ...
  try {
    ...
    // Mark as claimed
    await db.query('UPDATE parent_rewards SET claimed=true, claimed_at=NOW() WHERE id=$1', [reward.id]);
...
router.get('/admin/:id', requireAdmin, async (req, res, next) => {
  ...
  try {
    const { rows } = await db.query(
      'SELECT * FROM parent_study_modules WHERE id=$1',
      [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Not found' });
    res.json(rows[0]);
