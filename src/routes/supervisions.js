...
async function loadQuestionSet() {
  ...
  const { rows } = await db.query(
    `SELECT ordinal, question_key, category, question_text, is_required
     FROM ladn.supervision_question_templates
...
// Persist a targeted-summary object into ladn.supervision_structured (one row per question_key)
async function persistStructured(supervisionId, staffId, questions, summaryObj) {
  ...
  for (const q of questions) {
    ...
    await db.query(`
      INSERT INTO ladn.supervision_structured
...
router.post('/form/submit', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(`
      UPDATE ladn.supervisions SET pre_questionnaire_responses=$1, wellbeing_score=$2
...
router.get('/form/:token', async (req, res) => {
  try {
    ...
    const { rows } = await db.query(`
      ...
      FROM ladn.supervisions s
      JOIN ladn.staff st ON st.id = s.staff_id
      LEFT JOIN ladn.staff sv ON sv.id = s.supervisor_id
  ...
  try {
    ...
    const [overdueRes, signoffRes, actionsRes] = await Promise.all([
      ...
      db.query(`
        ...
        FROM ladn.staff s
        LEFT JOIN ladn.supervisions sv ON sv.staff_id=s.id AND sv.status='completed'
      ...
      db.query(`
        ...
        FROM ladn.supervisions sv
        JOIN ladn.staff st ON st.id=sv.staff_id
  ...
  try {
    ...
    let q = `
      SELECT sv.*, st.first_name || ' ' || st.last_name as staff_name,
        ...
        (SELECT COUNT(*) FROM ladn.supervision_targets t WHERE t.supervision_id=sv.id AND t.achieved=false) as outstanding_targets
      FROM ladn.supervisions sv
      JOIN ladn.staff st ON st.id = sv.staff_id
...
router.get('/staff-list', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(`
      SELECT s.id, s.first_name, s.last_name, s.role,
        (SELECT MAX(conducted_date) FROM ladn.supervisions WHERE staff_id=s.id AND status='completed') as last_supervision,
        (SELECT MIN(scheduled_date) FROM ladn.supervisions WHERE staff_id=s.id AND status='scheduled') as next_supervision,
        (SELECT COUNT(*) FROM ladn.supervision_targets WHERE staff_id=s.id AND achieved=false) as outstanding_targets,
        (SELECT id FROM ladn.supervisions WHERE staff_id=s.id AND status='scheduled' ORDER BY scheduled_date LIMIT 1) as next_supervision_id,
        EXTRACT(DAY FROM NOW() - (SELECT MAX(conducted_date) FROM ladn.supervisions WHERE staff_id=s.id AND status='completed')) as days_since_last
      FROM ladn.staff s
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(`
      INSERT INTO ladn.supervisions (staff_id, scheduled_date, supervisor_id, status, form_token, type, agenda_items)
    ...
    await db.query('UPDATE ladn.supervisions SET form_token=$1 WHERE id=$2', [realToken, sid]);
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (req.body.agreed_targets && Array.isArray(req.body.agreed_targets)) {
      const staffRow = await db.query('SELECT staff_id FROM ladn.supervisions WHERE id=$1', [req.params.id]);
      const staffId = staffRow.rows[0]?.staff_id;
      for (const t of req.body.agreed_targets) {
        if (t.text) {
          await db.query(`
            INSERT INTO ladn.supervision_targets (staff_id, supervision_id, target_text, area, due_date)
...
router.post('/:id/audio-upload', upload.single('audio'), async (req, res) => {
  ...
  try {
    const db = getPool();
    await db.query('UPDATE ladn.supervisions SET audio_recording_path=$1 WHERE id=$2', [relPath, req.params.id]);
    ...
    transcribeAudio(filePath).then(async transcript => {
      if (!transcript) return;
      await db.query('UPDATE ladn.supervisions SET transcript=$1, updated_at=NOW() WHERE id=$2', [transcript, req.params.id]);
      // Get supervision type for AI summary
      const { rows } = await db.query('SELECT type FROM ladn.supervisions WHERE id=$1', [req.params.id]);
      const svType = rows[0]?.type || 'monthly_1to1';
      const summary = await generateAISummary(transcript, svType);
      if (summary) {
        await db.query(`UPDATE ladn.supervisions SET
...
router.post('/:id/audio', async (req, res) => {
  ...
  try {
    const db = getPool();
    await db.query('UPDATE ladn.supervisions SET audio_url=$1 WHERE id=$2', [audio_url, req.params.id]);
...
router.post('/:id/ai-summary', async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows } = await db.query('SELECT transcript, discussion_notes, type FROM ladn.supervisions WHERE id=$1', [req.params.id]);
    if (!rows.length) return res.status(404).json({ error: 'Not found' });
    ...
    await db.query('UPDATE ladn.supervisions SET ai_summary=$1, ai_summary_generated_at=NOW() WHERE id=$2',
...
router.post('/:id/signoff', async (req, res) => {
  ...
  try {
    ...
    const { rows: pinRows } = await db.query(
      'SELECT id FROM ladn.staff WHERE id=$1 AND pin_hash=crypt($2, pin_hash)',
      [req.user.id, pin]
    );
    ...
    await db.query(
      `UPDATE ladn.supervisions SET ${field}=true, ${sigField}=NOW(), updated_at=NOW() WHERE id=$1`,
      [req.params.id]
    );
...
router.get('/:id/audio', async (req, res) => {
  try {
    const db = getPool();
    const { rows } = await db.query('SELECT audio_recording_path FROM ladn.supervisions WHERE id=$1', [req.params.id]);
...
router.post('/recording/:sid/audio-chunk',
  ...
  async (req, res) => {
    ...
    const { rows: svRows } = await db.query(
      'SELECT id, transcript FROM ladn.supervisions WHERE id=$1 AND supervisor_id=$2',
      [sid, req.user.id]
    );
...
router.post('/recording/:sid/finalize', async (req, res) => {
  ...
  const { rows: svRows } = await db.query(
    'SELECT id, transcript, supervisor_id FROM ladn.supervisions WHERE id=$1',
    [sid]
  );
...
router.post('/record', recordUpload.single('audio'), async (req, res) => {
  ...
  try {
    ...
    await db.query('UPDATE ladn.supervisions SET audio_recording_path=$1 WHERE id=$2', [relPath, sid]);
    ...
    (async () => {
      ...
      if (!transcript) {
        await db.query("UPDATE ladn.supervisions SET status='draft', updated_at=NOW() WHERE id=$1", [sid]);
        return;
      }
      await db.query('UPDATE ladn.supervisions SET transcript=$1, updated_at=NOW() WHERE id=$2', [transcript, sid]);
  ...
  try {
    ...
    const { rows } = await db.query(
      `SELECT question_key, category, summary_text, rag, flag, ordinal
       FROM ladn.supervision_structured WHERE supervision_id=$1 ORDER BY ordinal`,
      [req.params.id]
    );
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
  ...
  try {
    ...
    const { rows } = await db.query(`
      ...
      SELECT
        ...
        (SELECT summary_text FROM ladn.supervision_structured st WHERE st.supervision_id=ls.supervision_id AND st.question_key='wellbeing')      AS wellbeing,
        (SELECT rag          FROM ladn.supervision_structured st WHERE st.supervision_id=ls.supervision_id AND st.question_key='wellbeing')      AS wellbeing_rag,
        (SELECT summary_text FROM ladn.supervision_structured st WHERE st.supervision_id=ls.supervision_id AND st.question_key='key_children')   AS key_children,
        (SELECT summary_text FROM ladn.supervision_structured st WHERE st.supervision_id=ls.supervision_id AND st.question_key='parents')        AS parents,
        (SELECT summary_text FROM ladn.supervision_structured st WHERE st.supervision_id=ls.supervision_id AND st.question_key='sen')            AS sen,
        (SELECT summary_text FROM ladn.supervision_structured st WHERE st.supervision_id=ls.supervision_id AND st.question_key='workload')       AS workload,
        (SELECT summary_text FROM ladn.supervision_structured st WHERE st.supervision_id=ls.supervision_id AND st.question_key='cpd_review')     AS cpd,
        (SELECT summary_text FROM ladn.supervision_structured st WHERE st.supervision_id=ls.supervision_id AND st.question_key='targets_new')    AS smart_targets,
        COALESCE((SELECT bool_or(flag) FROM ladn.supervision_structured st WHERE st.supervision_id=ls.supervision_id AND st.question_key='safeguarding'), false) AS safeguarding_flag,
        (SELECT COUNT(*) FROM ladn.supervision_targets t WHERE t.staff_id=s.id AND t.achieved=false) AS outstanding_targets,
