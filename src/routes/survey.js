...
router.post('/submit', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(
      'INSERT INTO ladn.survey_responses (survey_type, responses, email) VALUES ($1,$2,$3) RETURNING id',
...
router.get('/results/:type', authenticate, async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows } = await db.query(
      'SELECT * FROM ladn.survey_responses WHERE survey_type=$1 ORDER BY submitted_at DESC',
      [req.params.type]
    );
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
...
router.get('/templates', authenticate, mgr, async (req, res) => {
  try {
    ...
    const { rows } = await db.query(`
      SELECT t.*,
        (SELECT COUNT(*) FROM ladn.survey_responses r WHERE r.template_id = t.id) AS response_count,
        (SELECT COUNT(*) FROM ladn.survey_invites i WHERE i.template_id = t.id) AS invite_count,
        (SELECT COUNT(*) FROM ladn.survey_invites i WHERE i.template_id = t.id AND i.opened_at IS NOT NULL) AS opened_count,
        (SELECT COUNT(*) FROM ladn.survey_invites i WHERE i.template_id = t.id AND i.clicked_at IS NOT NULL) AS clicked_count,
        (SELECT COUNT(*) FROM ladn.survey_invites i WHERE i.template_id = t.id AND i.response_id IS NULL AND i.sent_at IS NOT NULL) AS pending_count,
        (SELECT MAX(r.submitted_at) FROM ladn.survey_responses r WHERE r.template_id = t.id) AS last_response_at
      FROM ladn.survey_templates t
...
router.get('/templates/:id', authenticate, mgr, async (req, res) => {
  try {
    const { rows } = await getPool().query(
      'SELECT * FROM ladn.survey_templates WHERE id=$1', [req.params.id]
    );
...
router.post('/templates', authenticate, mgr, async (req, res) => {
  ...
  try {
    const { rows } = await getPool().query(
      `INSERT INTO ladn.survey_templates (name, slug, survey_type, description, questions)
...
router.put('/templates/:id', authenticate, mgr, async (req, res) => {
  ...
  try {
    const { rows } = await getPool().query(
      `UPDATE ladn.survey_templates SET
...
router.delete('/templates/:id', authenticate, mgr, async (req, res) => {
  try {
    await getPool().query(
      'UPDATE ladn.survey_templates SET active=false, updated_at=NOW() WHERE id=$1',
      [req.params.id]
    );
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
  ...
  try {
    const { rows } = await getPool().query(
      `SELECT r.*, i.email AS invite_email, i.clicked_at
       FROM ladn.survey_responses r
       LEFT JOIN ladn.survey_invites i ON i.response_id = r.id
...
router.post('/templates/:id/send', authenticate, mgr, async (req, res) => {
  ...
  try {
    const { rows: tpl } = await db.query(
      'SELECT * FROM ladn.survey_templates WHERE id=$1 AND active=true', [templateId]
    );
    ...
    const { rows: parents } = await db.query(`
      SELECT DISTINCT lower(trim(e)) AS email, child_id FROM (
        SELECT parent_1_email AS e, id AS child_id FROM ladn.children WHERE is_active=true AND parent_1_email IS NOT NULL AND parent_1_email != ''
        UNION ALL
        SELECT parent_2_email, id FROM ladn.children WHERE is_active=true AND parent_2_email IS NOT NULL AND parent_2_email != ''
    ...
    for (const p of parents) {
      ...
      const { rows: existing } = await db.query(
        'SELECT id FROM ladn.survey_invites WHERE template_id=$1 AND email=$2',
        [templateId, p.email]
      );
      if (existing.length) { skipped++; continue; }

      const { rows: inv } = await db.query(
        `INSERT INTO ladn.survey_invites (template_id, email, child_id, sent_at)
    ...
    for (const inv of invites) {
      ...
      try {
        await db.query(
          `INSERT INTO ladn.comms_email_queue
  ...
  try {
    const { rows } = await getPool().query(
      `SELECT name, slug, survey_type, description,
       ...
       FROM ladn.survey_templates WHERE active=true ORDER BY created_at`
    );
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/render/:slug', async (req, res) => {
  try {
    const { rows } = await getPool().query(
      'SELECT id, name, slug, survey_type, description, questions FROM ladn.survey_templates WHERE slug=$1 AND active=true',
      [req.params.slug]
    );
    ...
    if (token) {
      getPool().query(
        'UPDATE ladn.survey_invites SET clicked_at=COALESCE(clicked_at,NOW()) WHERE token=$1',
        [token]
      ).catch(() => {});
    }
...
router.post('/submit-template', async (req, res) => {
  ...
  try {
    const db = getPool();
    const { rows: tpl } = await db.query(
      'SELECT * FROM ladn.survey_templates WHERE slug=$1 AND active=true', [slug]
    );
    if (!tpl.length) return res.status(404).json({ error: 'Survey not found' });
    const template = tpl[0];

    const { rows } = await db.query(
      `INSERT INTO ladn.survey_responses (survey_type, responses, email, template_id)
    ...
    if (token) {
      await db.query(
        'UPDATE ladn.survey_invites SET response_id=$1, clicked_at=COALESCE(clicked_at,NOW()) WHERE token=$2',
        [responseId, token]
      ).catch(() => {});
    }
...
router.get('/open/:token', async (req, res) => {
  ...
  if (token) {
    getPool().query(
      'UPDATE ladn.survey_invites SET opened_at=COALESCE(opened_at,NOW()) WHERE token=$1',
      [token]
    ).catch(() => {});
  }
...
router.get('/templates/:id/results', authenticate, mgr, async (req, res) => {
  ...
  try {
    const { rows: tpl } = await db.query('SELECT * FROM ladn.survey_templates WHERE id=$1', [id]);
    if (!tpl.length) return res.status(404).json({ error: 'Not found' });
    const template = tpl[0];
    const questions = template.questions || [];

    const { rows: responses } = await db.query(
      'SELECT id, responses, email, submitted_at FROM ladn.survey_responses WHERE template_id=$1 ORDER BY submitted_at DESC',
      [id]
    );
    ...
    const { rows: [eng] } = await db.query(`
      ...
      FROM ladn.survey_invites WHERE template_id=$1
    `, [id]);
...
router.get('/templates/:id/export.csv', authenticate, mgr, async (req, res) => {
  ...
  try {
    ...
    const { rows: responses } = await db.query(
      'SELECT id, responses, email, submitted_at FROM ladn.survey_responses WHERE template_id=$1 ORDER BY submitted_at',
      [id]
    );
...
router.post('/templates/:id/remind', authenticate, mgr, async (req, res) => {
  ...
  const id = parseInt(req.params.id);
  try {
    const { rows: tpl } = await db.query('SELECT * FROM ladn.survey_templates WHERE id=$1 AND active=true', [id]);
    if (!tpl.length) return res.status(404).json({ error: 'Template not found or inactive' });
    const template = tpl[0];
    ...
    const { rows: pending } = await db.query(
      'SELECT * FROM ladn.survey_invites WHERE template_id=$1 AND response_id IS NULL AND sent_at IS NOT NULL',
      [id]
    );
    ...
    for (const inv of pending) {
      ...
      try {
        ...
        await db.query('UPDATE ladn.survey_invites SET reminded_at=NOW() WHERE id=$1', [inv.id]);
        reminded++;
  ...
  try {
    ...
    const { rows: [nlStats] } = await db.query(`
      SELECT
        ...
        (SELECT sent_to_count FROM ladn.newsletters WHERE sent_at IS NOT NULL ORDER BY sent_at DESC LIMIT 1) AS last_send_count,
        (SELECT COUNT(*) FROM ladn.newsletter_sends WHERE status='sent') AS total_sends,
        (SELECT COUNT(*) FROM ladn.newsletter_sends WHERE status='failed') AS total_failures
      FROM ladn.newsletters
    ...
    const { rows: surveyStats } = await db.query(`
      SELECT t.id, t.name, t.slug,
        ...
        (SELECT COUNT(*) FROM ladn.survey_invites i WHERE i.template_id=t.id AND i.clicked_at IS NOT NULL) AS clicked_count,
        (SELECT MAX(r.submitted_at) FROM ladn.survey_responses r WHERE r.template_id=t.id) AS last_response_at
      FROM ladn.survey_templates t WHERE t.active=true ORDER BY t.created_at
    `).catch(() => ({ rows: [] }));

    const { rows: [surveyTotal] } = await db.query(`
      SELECT COUNT(*) AS total_responses FROM ladn.survey_responses
    ...
    const { rows: [slipStats] } = await db.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM ladn.permission_slip_responses WHERE response='pending') AS pending_responses,
        (SELECT COUNT(*) FROM ladn.permission_slip_responses WHERE response='approved') AS signed_responses,
        (SELECT MAX(signed_at) FROM ladn.permission_slip_responses) AS last_signed_at
      FROM ladn.permission_slips
