...
// Assigning a pattern to a staff member SYNCS its days into ladn.staff_work_patterns
...
async function patternWithDays(db, id) {
  const { rows: p } = await db.query('SELECT * FROM ladn.work_patterns WHERE id=$1', [id]);
  if (!p.length) return null;
  const { rows: days } = await db.query(
    'SELECT day_of_week, shift_start, shift_end, is_off, break_minutes, room FROM ladn.work_pattern_days WHERE work_pattern_id=$1 ORDER BY day_of_week', [id]);
  return { ...p[0], days };
}
...
router.get('/', managerOnly, async (req, res) => {
  const db = getPool();
  try {
    const { rows } = await db.query('SELECT id FROM ladn.work_patterns ORDER BY is_default DESC, name');
    const out = [];
...
async function writeDays(db, patternId, days) {
  await db.query('DELETE FROM ladn.work_pattern_days WHERE work_pattern_id=$1', [patternId]);
  for (const d of (days || [])) {
    const dow = parseInt(d.day_of_week, 10);
    if (!(dow >= 0 && dow <= 6)) continue;
    await db.query(
      `INSERT INTO ladn.work_pattern_days (work_pattern_id, day_of_week, shift_start, shift_end, is_off, break_minutes, room)
...
router.post('/', managerOnly, async (req, res) => {
  ...
  try {
    ...
    if (b.is_default) await client.query('UPDATE ladn.work_patterns SET is_default=false WHERE is_default=true');
    const { rows } = await client.query(
      `INSERT INTO ladn.work_patterns (name, is_default, pattern_start_date, public_holiday_handling)
...
router.put('/:id', managerOnly, async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    if (b.is_default) await client.query('UPDATE ladn.work_patterns SET is_default=false WHERE is_default=true AND id<>$1', [req.params.id]);
    await client.query(
      `UPDATE ladn.work_patterns SET name=COALESCE($2,name), is_default=COALESCE($3,is_default),
...
router.delete('/:id', managerOnly, async (req, res) => {
  const db = getPool();
  try {
    await db.query('UPDATE ladn.staff SET work_pattern_id=NULL WHERE work_pattern_id=$1', [req.params.id]);
    await db.query('DELETE FROM ladn.work_patterns WHERE id=$1', [req.params.id]); // cascades to days
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/:id/assign', managerOnly, async (req, res) => {
  ...
  try {
    ...
    await client.query('BEGIN');
    await client.query('UPDATE ladn.staff SET work_pattern_id=$1 WHERE id=$2', [req.params.id, staffId]);
    // Close any prior open pattern rows so the engine doesn't see two patterns per day.
    await client.query(
      `UPDATE ladn.staff_work_patterns SET effective_to=$2::date - 1
    ...
    for (const d of pat.days) {
      await client.query(
        `INSERT INTO ladn.staff_work_patterns
