...
const MQTT_HOST   = process.env.MQTT_HOST  || '<server-ip>';
const MQTT_PORT   = parseInt(process.env.MQTT_PORT || '1883');
const SNAP_DIR    = process.env.DOORBELL_SNAP_DIR || '/app/data/doorbell-snapshots';
const SITE_BASE   = process.env.SITE_BASE || 'https://admin.example.com';
...
async function _handlePress(source) {
  ...
  try {
    const { rows } = await db.query(
      `INSERT INTO ladn.doorbell_events (source) VALUES ($1) RETURNING id`,
      [source]
    );
    eventId = rows[0].id;
  ...
  const _attachSnap = async () => {
    if (_snapBuffer && Date.now() - _snapBufferAt < 10_000) {
      ...
      if (sp) await db.query(`UPDATE ladn.doorbell_events SET snapshot_path=$1 WHERE id=$2`, [sp, eventId]).catch(() => {});
    }
  };
  if (_snapBuffer && Date.now() - _snapBufferAt < 10_000) {
    await _attachSnap();
