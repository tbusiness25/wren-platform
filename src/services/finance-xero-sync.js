...
async function refreshTokenIfNeeded(provider) {
  ...
  console.log('[xero-sync] refreshing OAuth token');
  ...
  const newExpiresAt = tokenSet.expires_at
  ...
  await db.query(`
    ...
    WHERE id=$4
  `, [encrypt(tokenSet.access_token), encrypt(tokenSet.refresh_token), newExpiresAt, provider.id]);
