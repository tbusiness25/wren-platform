...
function _haToken() {
  ...
  if (!t || t.startsWith('REPLACE_WITH')) throw new Error('HA_LADN_TOKEN not configured — see wren-docs/integrations/homeassistant-ladn.md');
  return t;
}
...
async function getHaConfig() {
  const db = getPool();
  const { rows } = await db.query('SELECT key, entity_id FROM ladn.ha_config');
...
async function saveHaConfig(key, entity_id) {
  const db = getPool();
  await db.query(
    `INSERT INTO ladn.ha_config (key, entity_id, updated_at)
     VALUES ($1, $2, NOW())
