 ...
 *
 * Every N seconds, scan ladn.tasks for due/in-progress items and fire a
...
function startTaskReminderDrainer({ interval = 30, cooldown = 600 } = {}) {
  ...
  async function drainOnce() {
    ...
    try {
      ...
      // For each, check last reminder latency
      const ids = dueTasks.map(t => t.id);
      const { rows: lastReminders } = await db.query(`
        SELECT payload->>'task_id' AS tid, MAX(queued_at) AS last_reminder
        FROM ladn.notification_queue
        WHERE event_type = 'task_reminder'
          AND (payload->>'task_id')::integer = ANY($1::integer[])
